...
"""
INTRO:
Mycroft plugin for the open source Sonos Controller (SoCo) module.
"""
import re
...
class SonosConnect(soco.SoCo):
    """ Implement the Sonos Controller API.
    """
    ...
    def status(self):
        """ Get current playback status (across the Sonos system) """
        try:
    ...
    def is_playing(self, device=None):
        """ Get playback state, either across Sonos or for given device.
        Args:
    ...
    def play(self, device, uris=None, context_uri=None):
        ...
        Args:
            ...
            context_uri (str): Sonos context uri for playing albums or
                               artists.
        """
        data = {}
        if uris:
    ...
    def pause(self, device):
        ...
        """
        LOG.debug('Pausing Sonos playback')
        try:
...
class SonosSkill(CommonPlaySkill):
    ...
    @intent_handler(IntentBuilder('').require('Transfer').require('Sonos')
                                     .require('ToDevice'))
